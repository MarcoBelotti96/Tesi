...
\vspace{12mm}
\begin{center}
{\huge{\bf Studio degli open data su meteo, aria e COVID\-19 in Lombardia ed analisi delle possibili relazioni}}\\
\vspace{4mm}
{\huge{\bf TITOLO}}\\
\vspace{4mm}
{\huge{\bf TITOLO}}\\
...
%\todo{atrent: andrà speigato bene cosa si intende col termine, qui lo metterei tra virgolette o in corsivo}
...
Nel capitolo 4 verranno presentate le analisi svolte sull'influenza del traffico sulle concentrazioni di tre inquinanti: ossidi di azoto, particolato atmosferico (PM10 e PM2.5) e monossido di carbonio.\todo{atrent: metti sempre i subscript per i numeri nelle formule chimiche, o almeno usa sempre lo stesso standard}
...
Di ogni modello sono state valuate le prestazioni grazie all'uso della tecnica del validation set, una tecnica molto conosciuta che grazie all'utilizzo di una parte dei dati iniziali, che viene sottratta casualmente a quelli usati per creare il modello, permette di verificare quanto siano precise le previsioni fatte da un modello confrontandole con valori reali.  %\todo{atrent: è automatica o devi scegliere tu il subset?}
...
\footnote{Il diametro aerodinamico di una particella è, per definizione, il diametro di una particella perfettamente sferica, di densità unitaria (1$g/cm^3$), che ha le stesse caratteristiche inerziali, overro che si deposita su un supporto alla stessa velocità}
...
, dovuto dalla quantità di attività antropiche (e in parte probabilmente anche da un fondo naturale) presenti nella nostra regione, oltre il quale sarà più complicato scendere ulteriormente senza radicali cambiamenti che riescano ad eliminare alcune delle principali fonti emissive. Infatti, se a partire dal 2004 la media annuale della serie normalizzata è calata costantemente di anno in anno, dal 2014 in poi i valori si sono stabilizzati sui 30$\mu g/m^3$. Dalla serie ottenuta si capisce che mediamente la situazione per quanto riguarda questo inquinante non è critica, poichè con condizioni climatiche medie le concentrazioni sarebbero sempre sotto ai limiti legislativi. A causare i superamenti, che avvengono quasi tutti nei mesi invernali, sono invece le condizioni climatiche sfavorevoli di quei mesi che, unitamente all'uso dei riscaldamenti domestici ed in generale ad un maggior consumo energetico, portano il particolato ad accumularsi in atmosfera senza riuscire a disperderlo.

\todo{atrent: da qualche parte definisci cosa si intende con "fondo naturale" e come si possa misurare? con refbib intendo}
\todo[color=green]{mbelotti: non riesco a trovare bib che diano una definizione, viene citato in molte ricerche ma ne non esiste una vera e propria definizione}
...
%\todo{sempre uno spazio tra la parola e la citazione}
...
Guardando sia i grafici delle concentrazioni reali che quello delle serie normalizzate si osserva come nei mesi di lockdown le concentrazioni abbiano subito un discreto ribasso, di circa il 20\%, rispetto agli anni precedenti, sicuramente dovuto al forte blocco del traffico e di alcune attività produttive.
...
\chapter{Appendice}
%citazione al repo dove sono disponibili tutti i notebook e tutto il materiale per riprodurre l'esperimento
